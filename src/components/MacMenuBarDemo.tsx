      ...
      <AnimatePresence>
        {showPopover && (
          <motion.div
            ...
            className="ml-auto w-72 origin-top-right shadow-xl"
          >
            <div className="frame-block px-4 py-3.5 border-t-0 rounded-t-none">

              {stage !== "logged-in" ? (
                /* LOGGED OUT — login form */
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <Onigiri size={32} emotion="neutral" />
                    <div>
                      <p className="font-serif text-sm text-warm-brown leading-tight">
                        {now.getHours() < 12 ? "Good morning!" : now.getHours() < 17 ? "Good afternoon!" : now.getHours() < 22 ? "Good evening!" : "Hey night owl!"}
                      </p>
                      <p className="text-[0.45rem] font-mono text-muted-ink/50">Log in to continue</p>
                    </div>
                  </div>
                  <motion.div
                  ...
                  >
                    <div className="space-y-2">
                      <div className="relative">
                        <input
                          ...
                          value={emailText}
                          readOnly
                          className="field-coral text-[0.6rem] w-full pr-7 h-8"
                        />
                        {stage === "typing-email" && (
                          <motion.span
                            className="absolute right-2 top-1/2 -translate-y-1/2 text-soft-coral text-[0.45rem] font-mono"
                      ...
                      <motion.div
                        className={`w-full font-mono text-[0.6rem] py-2 text-center border-2 transition-colors ${
                    ...
                    <p className="text-center text-[0.45rem] font-mono text-muted-ink/40 mt-2.5">
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    <Onigiri size={32} emotion="happy" />
                    <div>
                      <p className="font-serif text-sm text-warm-brown leading-tight">Welcome back, Alice!</p>
                      <p className="text-[0.45rem] font-mono text-muted-ink/50">What do you want to do?</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2.5 w-full font-mono text-[0.55rem] text-warm-paper bg-soft-coral py-2 px-3 border-2 border-warm-brown text-left hover:opacity-90 transition-opacity cursor-default">
                      <span className="text-xs">✏️</span>
                      <span>Create a new log</span>
                    </div>
                    <div className="flex items-center gap-2.5 w-full text-[0.5rem] font-mono text-muted-ink/60 hover:text-soft-coral transition-colors py-1.5 px-3 text-left cursor-default">
                      <span className="text-xs">📊</span>
                      <span>View my dashboard →</span>
                    </div>
                  </div>
                  <div className="mt-3 pt-2.5 border-t border-warm-brown/15 text-[0.4rem] font-mono text-muted-ink/30 flex items-center gap-1">
                    <motion.span
